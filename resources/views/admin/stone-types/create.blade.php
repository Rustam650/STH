...
@section('title', 'Добавление вида камня')

@section('header', 'Добавление нового вида камня')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow-lg p-6">
        <form action="{{ route('admin.stone-types.store') }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="full_description" id="full_description" rows="6" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('full_description') }}</textarea>
            ...
            <div class="mb-4">
                ...
                <input type="url" name="image" id="image" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('image') }}" required>
                @error('image')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
